...
with DAG(
    ...
    def load_data_from_staging(**kwargs):
        ...
        Applies SCD2 logic or simple INSERT depending on the use_scd2 flag.
        """
        ...
        table_config = {
            ...
            "TRIPS": {"pks": ["TRIP_ID"], "use_scd2": True},
            "STOPS": {"pks": ["STOP_ID"], "use_scd2": True},
            "CALENDAR": {"pks": ["SERVICE_ID"], "use_scd2": True},
        ...
        for table, config in table_config.items():
            ...
            if use_scd2:
                ...
                update_sql = f"""
                    UPDATE {table} tgt
                    SET tgt.VALID_TO = '{logical_date}',
                        tgt.IS_ACTIVE = FALSE
                    FROM {stg_table} stg
                    WHERE {join_conditions}
                      AND tgt.IS_ACTIVE = TRUE
                      AND HASH(tgt.*) != HASH(stg.*)
                """
                ...
                insert_sql = f"""
                    ...
                    SELECT {cols_str}, '{logical_date}', NULL, TRUE
                    FROM {stg_table} stg
                    WHERE NOT EXISTS (
